  ...
  return (
    <div className="p-4 sm:p-6 max-w-6xl" data-testid="notification-management">
      <div className="mb-6">
        ...
        {!isSuper && (
          <p className="mt-2 text-xs text-amber-700 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2" data-testid="notif-watcher-banner">
            You are in watcher mode — sensitive values (OTPs, contact names, phone numbers) are masked, and any change action will be blocked by the super admin.
          </p>
        )}
      </div>
      ...
      {activeTab === "campaigns" && <CampaignTab authHeaders={authHeaders} isSuper={isSuper} onOpenConvo={(phone) => { setActiveTab("conversations"); setConvoPhone(phone); }} />}
      {activeTab === "auto" && <AutoResponseTab authHeaders={authHeaders} isSuper={isSuper} />}
      {activeTab === "triggers" && <TriggersTab authHeaders={authHeaders} isSuper={isSuper} />}
      {activeTab === "templates" && <TemplatesTab authHeaders={authHeaders} isSuper={isSuper} />}
      {activeTab === "conversations" && <ConversationsTab authHeaders={authHeaders} initialPhone={convoPhone} isSuper={isSuper} />}
      {activeTab === "otplogs" && <OTPLogsTab authHeaders={authHeaders} isSuper={isSuper} />}
    </div>
  );
}
...
/*                     CONVERSATIONS TAB                          */
/* ═══════════════════════════════════════════════════════════════ */
function ConversationsTab({ authHeaders, initialPhone, isSuper = true }) {
              ...
              <button key={c.id} onClick={() => openConvo(c.phone)} data-testid={`convo-${c.phone}`}
                ...
                <div className="relative">
                  <div className="w-11 h-11 rounded-full bg-[#0B1C3D] flex items-center justify-center text-white font-bold text-sm shrink-0">
                    {isSuper ? (c.contact_name || c.phone || "?").charAt(0).toUpperCase() : "•"}
                  </div>
                  {c.unread_count > 0 && (
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex justify-between items-center">
                    <p className={`text-sm truncate ${c.unread_count > 0 ? "font-bold text-[#0B1C3D]" : "font-medium text-gray-800"}`}>
                      {isSuper
                        ...
                        : <span className="text-gray-500 italic">Hidden conversation</span>}
                    </p>
                  ...
                  <p className={`text-xs truncate mt-0.5 ${c.unread_count > 0 ? "text-gray-700 font-medium" : "text-gray-500"}`}>
                    {isSuper ? (c.last_message || "No messages") : <span className="italic">Protected message content</span>}
                  </p>
                </div>
...
/*                     OTP LOGS TAB                               */
/* ═══════════════════════════════════════════════════════════════ */
function OTPLogsTab({ authHeaders, isSuper = true }) {
          ...
          {loading ? <p className="text-gray-400 text-center py-8 text-sm">Loading...</p> :
            ...
            logs.map((log, i) => (
              <div key={log.mobile + i} className="grid grid-cols-12 gap-2 px-4 py-3 text-sm hover:bg-gray-50 items-center">
                <div className="col-span-3 font-medium text-[#0B1C3D]">
                  {isSuper
                    ? log.mobile?.replace(/^\++/, "+")
                    : (log.mobile ? `+${log.mobile.replace(/^\++/, "").slice(0, 2)}${"•".repeat(Math.max(0, log.mobile.replace(/^\++/, "").length - 4))}${log.mobile.slice(-2)}` : "—")}
                </div>
                <div className="col-span-3 text-gray-700 truncate">
                  {isSuper
                    ? (log.guest_name || <span className="text-gray-300 italic">Not registered</span>)
                    : (log.guest_name ? <span className="text-gray-400 italic">Hidden</span> : <span className="text-gray-300 italic">Not registered</span>)}
                </div>
                <div className="col-span-2">
                  <code className="bg-gray-100 px-2 py-0.5 rounded text-xs font-mono">{isSuper ? log.otp : "••••••"}</code>
                </div>
                <div className="col-span-2">
                  {log.verified ? (
